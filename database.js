...
class GitHubDatabase {
    ...
    // Принудительное создание нового файла (обход конфликта)
    async forceCreateNewFile(journalData) {
        if (!this.hasToken()) {
            throw new Error('GitHub token не установлен');
        }

        try {
            // Сначала пытаемся удалить существующий файл
            try {
                const currentFileResponse = await fetch(
                    `https://api.github.com/repos/${this.owner}/${this.repo}/contents/${this.dataFile}`,
                    {
                ...
                if (currentFileResponse.ok) {
                    ...
                    const deleteResponse = await fetch(
                        `https://api.github.com/repos/${this.owner}/${this.repo}/contents/${this.dataFile}`,
                        {
                            ...
                            body: JSON.stringify({
                                message: `🔄 Пересоздание файла данных: ${new Date().toLocaleString('ru-RU')}`,
                                branch: this.branch,
                                sha: data.sha
                            })
                        }
                    ...
                    if (!deleteResponse.ok) {
                        console.warn('Не удалось удалить старый файл, продолжаем...');
                    } else {
            ...
            const response = await fetch(
                `https://api.github.com/repos/${this.owner}/${this.repo}/contents/${this.dataFile}`,
                {
                    ...
                    body: JSON.stringify({
                        message: `🔄 Создание нового файла данных: ${new Date().toLocaleString('ru-RU')}`,
                        content: content,
                        branch: this.branch
                        // Не передаем SHA для создания нового файла
                    })
                }
            );
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                throw new Error(`Ошибка создания файла: ${response.status} - ${errorData.message || response.statusText}`);
            }

            console.log('Новый файл данных успешно создан в GitHub');
            return true;
        } catch (error) {
            console.error('Ошибка принудительного создания файла:', error);
            throw error;
        }
    }
}
